...
export const migratePrograms = async (subdomain: string | undefined) => {
  ...
  try {
    ...
    const itemTypeId = (await itemTypeToId('program')).id
    const blockIds = await allBlockIds()
    const allPosts = await allPages(wpapi, 'event')
    fs.writeFileSync(`./lib/scripts/migration/data/${subdomain}-program.json`, JSON.stringify(allPosts, null, 2))
    // Main artists
    return
